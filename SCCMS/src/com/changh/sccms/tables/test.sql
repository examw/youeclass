...
select t.* from tbTrade t,tbStudent s where t.stu_id = s.stu_id and s.stu_username = 'hahaha' order by t.trade_time desc;  sds

select *  from tbExamCategory where exam_ename is null

update tbExamCategory set exam_ename = 'xfgc' where exam_ename is null